...
{
    public Text objectCount, currentCount;
    public ComputeShader cullingComputeShader;
    public List<RenderGroup> renderGroups = new List<RenderGroup>();
    public bool viewCulling = true;

    public float DistanceCulling{get{ return renderGroups[0].farClipDistance; }set { renderGroups[0].farClipDistance = value; } }
    ...
    public class RenderGroup
    {
        ...
        public ComputeBuffer visibleInstanceOnlyTransformBuffer;
        public ComputeBuffer visibleTransformIDBuffer;
    ...
    void LateUpdate()
    {
        ...
        AsyncGPUReadback.Request(renderGroups[0].visibleTransformIDBuffer, r =>
        {
            if (r.hasError)
            {
                return;
            ...
            uint[] bs = new uint[d.Length];
            d.CopyTo(bs);
            count = bs.ToList().FindAll(b => b == 1).Count;
        });
