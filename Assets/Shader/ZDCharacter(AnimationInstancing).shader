Shader "ZDShader/LWRP/Character(AnimationInstancing)"
{
    Properties
    {
        _VertexDataMap ("Vertex Data Map", 2D) = "Bump" { }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert(appdata v, uint id: SV_VERTEXID)
            {
                ...
                #ifdef _DrawMeshInstancedProcedural
                    float3 objectPosition = frac(mul(_ObjectToWorldBuffer[mid], float4(0.0, 0.0, 0.0, 1.0)).xyz);
                #else
                    float3 objectPosition = frac(mul(GetObjectToWorldMatrix(), float4(0.0, 0.0, 0.0, 1.0)).xyz);
                #endif
                _Time.y += objectPosition.x * objectPosition.z;
                
                float d = 0.3333333333333333;
                float t1 = fmod(_Time.y * 0.5, d);
                float t2 = fmod(_Time.y * 0.5, d) + d;
                float index = (id + 0.5) * _VertexDataMap_TexelSize.y;
                
                float4 positionOS = tex2Dlod(_VertexDataMap, float4(t1, index, 0, 0));
                float4 normalOS = tex2Dlod(_VertexDataMap, float4(t2, index, 0, 0));
                ...
                RTD_OL *= (lerp(widthRange.x, widthRange.y, saturate(dist - 0.05) * widthRange.z));
                    ...
                    o.vertex = TransformWorldToHClip(float4(positionWS, 1).xyz);
                #else
        ...
        Pass
        {
            ...
            #endif
            Varyings LitPassVertex(Attributes input, uint id: SV_VERTEXID)
            {
                ...
                float t1 = fmod(_Time.y * 0.5, d);
                float t2 = fmod(_Time.y * 0.5, d) + d;
                float t3 = fmod(_Time.y * 0.5, d) + d * 2.0;
                float index = (id + 0.5) * _VertexDataMap_TexelSize.y;
                
                float4 positionOS = tex2Dlod(_VertexDataMap, float4(t1, index, 0, 0));
                float4 normalOS = tex2Dlod(_VertexDataMap, float4(t2, index, 0, 0));
                float4 tangentOS = tex2Dlod(_VertexDataMap, float4(t3, index, 0, 0));
                ...
                VertexNormalInputs normalInput;
                    ...
                    normalInput = InitVertexNormalInputs(normalOS.xyz, tangentOS, mid);
                #else
                    vertexInput = GetVertexPositionInputs(positionOS.xyz);
                    normalInput = GetVertexNormalInputs(normalOS.xyz, tangentOS);
                #endif
        ...
        Pass
        {
            ...
            float4 GetShadowPositionHClip(Attributes input, uint id, uint mid)
            {
                #ifdef _DrawMeshInstancedProcedural
                    float3 objectPosition = frac(mul(_ObjectToWorldBuffer[mid], float4(0.0, 0.0, 0.0, 1.0)).xyz);
                #else
                ...
                #ifdef _DrawMeshInstancedProcedural
                    float3 positionWS = mul(_ObjectToWorldBuffer[mid], float4(positionOS.xyz, 1.0));
                    float3 normalWS = mul(_ObjectToWorldBuffer[mid], float4(normalOS.xyz, 0.0));
                #else
            ...
            Varyings ShadowPassVertex(Attributes input, uint id: SV_VERTEXID)
            {
                Varyings output = (Varyings)0;
                #ifdef _DrawMeshInstancedProcedural
                    uint mid = _VisibleInstanceOnlyTransformIDBuffer[input.mid];
                ...
                output.positionCS = GetShadowPositionHClip(input, id, mid);
                return output;
            }
        ...
        Pass
        {
            ...
            Varyings DepthOnlyVertex(Attributes input, uint id: SV_VERTEXID)
            {
                Varyings output = (Varyings)0;
    ...
    // same properties.
    CustomEditor "UnityEditor.Rendering.Funcy.LWRP.ShaderGUI.ZDCharacter_AnimationInstancing"
}